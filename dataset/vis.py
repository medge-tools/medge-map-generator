...
class DatasetVis():
    def add_handle(self, context):
        ...
        draw_handle_post_pixel = SpaceView3D.draw_handler_add(
            self.draw_callback_post_pixel,(context,), 'WINDOW', 'POST_PIXEL')
        draw_handle_post_view = SpaceView3D.draw_handler_add(
            self.draw_callback_post_view,(context,), 'WINDOW', 'POST_VIEW')
    ...
    def draw_callback_post_pixel(self, context: Context):
        # Validate
        obj = context.object
        if not obj: return
        mesh = obj.data
    ...
    def draw_callback_post_view(self, context: Context): 
        # Validate
        obj = context.object
        if not obj: return
        mesh = obj.data
